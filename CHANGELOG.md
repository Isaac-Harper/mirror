...
Jar versions carry the Minecraft version as build metadata (for example `0.1.2+26.1.2`); this file tracks the mod version for the 26.1 line (branch `mc-26.1`). The `publishMods` task publishes the matching `## <version>` section below as the Modrinth changelog.

## 0.1.2

Brings the 26.1 build up to parity with 0.1.2 for 26.2:
...
- Reflections are now hidden behind the item in your hand instead of drawing over it.
- Mirrors seen inside another mirror's reflection keep their frame.
